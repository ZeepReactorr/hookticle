...
def sci(keywords):
    ...
    keywords = ['No keywords in article'] + keywords
    ...
    for key, res in dico_keywords.items():
        print(key, res)
    print(f"articles not found : {article_not_found}", f"articles not accessible : {article_not_accessible}", sep="\n")
...
if __name__ == "__main__":
    ...
    print(f'Figure is ready. Processing the articles took {(end-start)//60} minutes and {np.round(((end-start)/60-(end-start)//60)*60)}')
